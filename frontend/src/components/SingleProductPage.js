...
const SingleProductPage = () => {
      ...
      if (productResponse.status === 'fulfilled') {
        ...
        if (productData.category) {
          try {
            ...
            const filtered = relatedResponse.products?.filter(p => p._id !== id) || [];
  ...
  const calculatePrice = () => {
    ...
    if (silverPrice && product.weightInTola && product.makingCost) {
      const totalPrice = (silverPrice.pricePerTola * product.weightInTola + product.makingCost) * quantity;
  ...
  const handleAddToCart = async () => {
    try {
      ...
      await ApiService.addToCart(sessionId, product._id, quantity, customization || null);
      ...
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center max-w-md mx-auto">
          ...
          <div className="space-y-3">
            ...
            <button onClick={() => navigate(-1)} className="btn btn-secondary block">
              Go Back
            </button>
          </div>
        </div>
        ...
        <div className="grid lg:grid-cols-2 gap-12">
              ...
              <div className="grid grid-cols-4 gap-2">
                {product.additionalImages.map((image, index) => (
                  <button
                    ...
                    className={`card overflow-hidden ${selectedImage === index + 1 ? 'ring-2 ring-saffron' : ''}`}
                  >
                    <img
                      src={image}
          ...
          <div className="space-y-6">
                ...
                <div>
                  ...
                  {silverPrice && product.category?.name.includes('Silver') && (
                    <p className="text-xs mt-1" style={{ color: 'var(--stone-gray)' }}>
                      Silver rate: Rs. {silverPrice.pricePerTola?.toLocaleString()}/tola
                    </p>
                  )}
                </div>
            ...
            <div>
              ...
              <p className="leading-relaxed" style={{ color: 'var(--stone-gray)' }}>
                {product.description}
              </p>
            </div>
            ...
            <div>
              ...
              <div className="space-y-2">
                <div className="flex justify-between">
                  ...
                  <span style={{ color: 'var(--dark-gray)' }}>{product.category?.name}</span>
                </div>
                {product.weightInTola && (
                  <div className="flex justify-between">
                    <span style={{ color: 'var(--stone-gray)' }}>Weight:</span>
                    <span style={{ color: 'var(--dark-gray)' }}>{product.weightInTola} tola</span>
                  </div>
                )}
                {product.height && (
                  <div className="flex justify-between">
                    <span style={{ color: 'var(--stone-gray)' }}>Height:</span>
                    <span style={{ color: 'var(--dark-gray)' }}>{product.height}</span>
                  </div>
                )}
                {product.makingCost && (
                  <div className="flex justify-between">
                    <span style={{ color: 'var(--stone-gray)' }}>Making Cost:</span>
                    <span style={{ color: 'var(--dark-gray)' }}>Rs. {product.makingCost.toLocaleString()}</span>
                  </div>
                )}
              </div>
            </div>
