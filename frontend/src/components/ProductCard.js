...
export default function ProductCard({ image, title, category, price, product }) {
  // Function to generate WhatsApp order link
  const generateWhatsAppOrderLink = () => {
    const adminPhone = process.env.REACT_APP_WHATSAPP_PHONE || '977XXXXXXXXX'; // Default or from env
...
Thank you!`;
  ...
  return (
    <div className="w-full max-w-sm bg-white border border-white shadow-sm rounded-sm overflow-hidden hover:shadow-xl transition-shadow duration-300 ease-in-out">
      <img className="p-4 rounded-t-lg w-full h-[400px] object-cover" src={image} alt={title} />
      <div className="px-5 pb-5">
        <h5 className="text-lg font-body font-semibold tracking-tight text-black">{title}</h5>
        <p className="text-sm text-ggrey font-heading mb-2">{category}</p>
        <div className="flex items-center justify-between">
          <span className="text-xl font-heading font-semibold text-black">{price}</span>
          <button 
